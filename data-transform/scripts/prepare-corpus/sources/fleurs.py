...
from ..common import CanonicalInputRow, CorpusStructuralError, canonical_split
...
def read_fleurs(root: Path) -> Iterator[CanonicalInputRow]:
    ...
    for tsv_path in tsv_files:
        ...
        if not audio_dir.is_dir():
            raise CorpusStructuralError(f"FLEURS_SCHEMA:MISSING_SPLIT_AUDIO:{upstream_split}")
        with tsv_path.open("r", encoding="utf-8", newline="") as handle:
            for line_number, columns in enumerate(csv.reader(handle, delimiter="\t"), start=1):
                if len(columns) != 7:
                    raise CorpusStructuralError(f"FLEURS_SCHEMA:ROW_{line_number}_HAS_{len(columns)}_FIELDS")
                sentence_id, audio_filename, raw_transcription, normalized_transcription, characterized_transcription, num_samples_raw, gender = columns
                ...
                try:
                    ...
                    raise CorpusStructuralError(f"FLEURS_SCHEMA:INVALID_NUM_SAMPLES:{source_key}") from error
