...
def read_shrutilipi(root: Path) -> Iterator[CanonicalInputRow]:
    ...
    for parquet_path in parquet_paths:
        ...
        if missing:
            raise CorpusStructuralError(f"SHRUTILIPI_SCHEMA:MISSING_COLUMNS:{','.join(sorted(missing))}")
        for batch in parquet.iter_batches():
            for row in batch.to_pylist():
                audio = row["audio_filepath"]
                if not isinstance(audio, dict):
                    raise CorpusStructuralError("SHRUTILIPI_SCHEMA:AUDIO_FIELD_CHANGED")
                upstream_path = audio.get("path")
                if not isinstance(upstream_path, str) or not upstream_path:
                    raise CorpusStructuralError("SHRUTILIPI_SCHEMA:MISSING_AUDIO_PATH")
