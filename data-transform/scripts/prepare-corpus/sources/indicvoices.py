...
def split_from_path(path: Path) -> tuple[str, str]:
    for part in reversed(path.parts):
        match = re.search(r"(?:^|[-_.])(train|test|dev|valid|validation)(?:[-_.]|$)", part.lower())
        if match:
...
def read_indicvoices(root: Path) -> Iterator[CanonicalInputRow]:
    ...
    required = {"audio_filepath", "text", "duration", "lang", "verbatim", "normalized"}
    for parquet_path in parquet_paths:
        ...
        if missing:
            raise CorpusStructuralError(f"INDICVOICES_SCHEMA:MISSING_COLUMNS:{','.join(sorted(missing))}")
        for batch in parquet.iter_batches():
            for row in batch.to_pylist():
                audio = row["audio_filepath"]
                if not isinstance(audio, dict):
                    raise CorpusStructuralError("INDICVOICES_SCHEMA:AUDIO_FIELD_CHANGED")
                upstream_path = audio.get("path")
                if not isinstance(upstream_path, str) or not upstream_path:
                    raise CorpusStructuralError("INDICVOICES_SCHEMA:MISSING_AUDIO_PATH")
                ...
                yield CanonicalInputRow(
                    ...
                    source_metadata={**{key: value for key, value in row.items() if key != "audio_filepath"}, "upstreamAudioPath": upstream_path},
                )
