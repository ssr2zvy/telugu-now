...
export function navigateNext(code: string, visible: boolean): ProfileStateResponse {
  ...
  const consumeTransaction = db.transaction(() => {
    ...
    // The first-time display consumes one future slot. Reserve its replacement in
    // this same transaction so a crash cannot commit consumption without replacement.
