...
class PreparationService {
  ...
  private async prepareOne(row: PendingRow): Promise<void> {
    ...
    const claimed = db.prepare(`
      UPDATE observations
      SET status = 'preparing', request_started_at = ?
    ...
    try {
      ...
      const prepared = await source.prepare({ sourceKey: row.source_key });
      ...
      db.prepare(`
        ...
        SET status = 'ready',
            ...
            request_duration_ms = ?
        WHERE id = ?
      `).run(prepared.text, completedAt, completedAt, completedAt - startedAt, row.id);
    } catch (error) {
      console.error('Observation preparation failed; returning it to pending.', error);
      db.prepare(`
        UPDATE observations
        SET status = 'pending', request_started_at = NULL
        WHERE id = ?
      `).run(row.id);
