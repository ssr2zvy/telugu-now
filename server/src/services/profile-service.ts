...
import { addRollingReplenishment, ensureLaunchQueue } from './queue-service';
import { preparationService } from './preparation-service';

interface ProfileRow {
  code: string;
  current_position: number | null;
  consumed_since_replenishment: number;
...
function assertValidProfileCode(code: string): void {
  if (!config.profileCodes.has(code)) throw new InvalidProfileCodeError('Invalid profile code.');
}

function ensureProfileRow(code: string): void {
  const now = Date.now();
  db.prepare(`
    INSERT INTO profiles (code, current_position, consumed_since_replenishment, created_at, updated_at)
    VALUES (?, NULL, 0, ?, ?)
...
function profileRow(code: string): ProfileRow {
  const row = db.prepare(`
    SELECT code, current_position, consumed_since_replenishment, last_client_seen_at
...
function historyLength(code: string): number {
  return (db.prepare('SELECT COUNT(*) AS count FROM history_entries WHERE profile_code = ?').get(code) as CountRow).count;
}
...
function currentObservation(code: string, currentPosition: number | null): DisplayObservation | null {
  ...
  const row = db.prepare(`
    SELECT o.id, o.source_id, o.source_key, o.text,
           o.group_kind, o.group_size, o.group_position,
...
function queueSummary(code: string): QueueSummary {
  ...
  const summary: QueueSummary = { unseenCount: 0, readyCount: 0, preparingCount: 0, pendingCount: 0 };
...
export function navigateNext(code: string, visible: boolean): ProfileStateResponse {
  ...
  const consumeTransaction = db.transaction(() => {
    ...
    db.prepare(`
      ...
      SET current_position = ?,
          consumed_since_replenishment = consumed_since_replenishment + 1,
          last_client_seen_at = ?,
          updated_at = ?
      WHERE code = ?
  ...
  const updated = profileRow(code);
  if (updated.consumed_since_replenishment >= 5) {
    addRollingReplenishment(code);
    db.prepare(`
      UPDATE profiles
      SET consumed_since_replenishment = 0, updated_at = ?
      WHERE code = ?
    `).run(Date.now(), code);
  }
