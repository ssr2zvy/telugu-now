...
class SourceRecordService {
  ...
  async resolve(sourceId: string, sourceKey: string): Promise<ResolvedSourceRecord> {
    ...
    if (existing) {
      ...
      try {
        ...
        media = parsed.length > 0 ? parsed : [{ kind: 'text', language: 'te', text: existing.text }];
      } catch {
        media = [{ kind: 'text', language: 'te', text: existing.text }];
      }
      return {
        sourceId,
        sourceKey,
  ...
  private async fetchAndCache(sourceId: string, sourceKey: string): Promise<FreshResolution> {
    ...
    const source = sourceId === this.legacyIteration1Resolver.id ? this.legacyIteration1Resolver : sourceRegistry.get(sourceId);
    const prepared = await source.prepare({ sourceKey });
    const requestCompletedAt = Date.now();
    const media = prepared.media && prepared.media.length > 0 ? prepared.media : [{ kind: 'text', language: 'te', text: prepared.text }];
    ...
    `).run(sourceId, sourceKey, prepared.text, JSON.stringify(media), requestCompletedAt);
