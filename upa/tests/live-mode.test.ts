...
test('live worker, repeated observations, immediate mastery, cycles and export',{timeout:90000},async()=>{
 ...
 try{
  ...
  const diagnostics=await import('../server/src/parsing/diagnostics');const d=diagnostics.parsingDiagnostics('001');assert.equal(d.levels[0]?.mastered,3);assert.equal(d.cache?.checked,5);assert.equal(d.worker.phase,'ready');assert.equal(d.queue.depth,0);assert.equal(d.selectionPolicy,'shortest-codepoints-v1');assert.ok(d.currentChain);assert.equal(d.currentChain.steps.length,1);assert.equal(d.currentChain.steps[0]?.targetId,ids[4]);
  let text='';for await(const chunk of diagnostics.diagnosticExport('001').chunks)text+=chunk;
  const out=JSON.parse(text);assert.equal(out.cycles.length,d.cycles.total);assert.equal(out.searches.length,searches.length);assert.equal(out.observations.length,17);assert.equal(out.selections.length,17);assert.equal(out.acquisitions.length,17);assert.equal(out.history.length,17);assert.equal(out.version,2);assert.ok(out.observations.every((o:any)=>o.selection_snapshot.matchedTargets.some((t:any)=>t.id===o.target_id)));assert.ok(out.observations.every((o:any)=>o.selection_snapshot.wordSelection==='shortest-codepoints-v1'));assert.ok(out.observations.filter((o:any)=>o.core===1).every((o:any)=>o.selection_snapshot.observationSelection.policy==='core1-shortest-five-v1'));assert.ok(out.observations.filter((o:any)=>o.core>1).every((o:any)=>o.selection_snapshot.observationSelection.policy==='all-matching-random-v1'));assert.equal(new Set(out.observations.map((o:any)=>o.source_key)).size,5);assert.ok(out.summary.targets.every((t:any)=>t.matchedWords>=1));
  // An older observation with no live-selection row must still be exported.
  db.prepare("INSERT INTO observations(id,source_id,source_key,text,status,selected_at,group_id,group_kind,group_size,group_position) VALUES('historic','fleurs-te','0','Saved older text','ready',0,'old','launch-fill',1,1)").run();
  db.prepare("INSERT INTO observation_acquisitions(observation_id,profile_code,acquisition_number,trigger_kind,triggered_at) VALUES('historic','001',18,'initial-fill',0)").run();
  let expanded='';for await(const chunk of diagnostics.diagnosticExport('001').chunks)expanded+=chunk;
  const all=JSON.parse(expanded);assert.equal(all.observations.length,18);assert.equal(all.acquisitions.length,18);assert.equal(all.selections.length,17);
  assert.equal(all.observations.find((o:any)=>o.id==='historic').text,'Saved older text');
