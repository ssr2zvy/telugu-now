...
Under **Display > Appearance > Control darkness**, a saved 0-60% slider darkens all app controls together, including audio paints, the reader's Settings button, and Settings buttons, inputs, and sliders. The default is 15% darker; 0% restores the original brightness. The same brightness multiplier preserves gradient positions, colors' relative shading, and opacity differences without dimming the page background or reading text. The magnifier seek cursor and highlighted section receive an additional 20% brightness reduction. **Reset control darkness** restores 15% independently of other appearance choices.

Under **Display > Appearance > Audio controls**, **Show audio timestamp** is off by default, including for existing profiles without this preference. Turning it on restores the timestamp next to the audio bar; hiding it removes its row and internal spacing, while screen readers still receive the precise seek time.

Under **Display > Appearance > Control spacing**, two independent 0-48 pixel sliders set **Audio bar to timestamp** and **Timestamp to magnifier** spacing (default 1px each). With the timestamp hidden, only **Audio bar to magnifier** is shown, using the first gap; the second gap remains saved but unused. A live preview mirrors timestamp visibility, the audio gradient, darkness, and **Magnifier position > Above / Below**. When shown, the order from the main bar outward is timestamp, then magnifier; switching sides or hiding the timestamp preserves both gaps. Existing shared spacing is carried over to both controls. **Reset control spacing** restores both defaults without changing position.
...
outside it and rejects sharing a file between the databases. Relative
