...
A single tap in the upper two thirds of the reader toggles playback. A single tap in the bottom third dismisses the magnifier and its actions first; otherwise it toggles the audio bar. These actions wait for the double-tap decision window, including at hitbox boundaries. The middle third's double tap toggles the Settings icon unless the tap hits a word. Showing or hiding the bar never changes playback. Settings uses the same corner placement as the Settings-language control.
...
Playback speed supports 0.1x-1.5x. Speed and bookmarks have 48 by 44 pixel targets below the lens. The precision scrubber moves one millisecond per pointer pixel. Reader gestures, including navigation doubles, remain available while precision controls are open; the bottom-third single tap dismisses the whole group. Direct interactions with sliders and action buttons remain dedicated to those controls.
The precision bar and its speed/bookmark actions start hidden. A 300 ms hold on the main seek bar, or a pressure-sensitive hard press, opens the group. A normal bar click seeks. Enter/Space on the bar provides keyboard access; Escape, a bottom-third single tap, or fading the controls dismisses the precision group. Merely opening or dismissing it does not pause audio. Precision dragging or arrow-key seeking still pauses for accuracy; an upper-screen single tap resumes. Space/Enter with the reader itself focused also toggles playback.
