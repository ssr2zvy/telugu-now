...
Under **Display > Appearance > Position**, separate text and audio-bar sliders adjust their vertical offsets from the defaults, from -200 to +200 pixels. Negative values move up; positive values move down. The player now starts lower: its bottom inset is 16 pixels plus the device safe area instead of reserving an additional 48-64 pixels below the player. **Audio control order** explicitly offers **Play above / magnifier below** (default) and **Play below / magnifier above**. Play spans the full player grid and is centered on the page, not just on the shorter seek track. Playback speed and bookmark buttons remain beside the main seek bar. Space is reserved for precision seeking so opening it does not shift the main bar under the pointer. Saved offsets and control order remain intact. Positions are constrained to the viewport, and long text is refitted to avoid overlapping audio controls. **Reset positions** restores both offsets to zero and Play above / magnifier below.
...
- Settings & popovers: the surface behind Settings and export dialogs. Automatic selects a light neutral for dark text or a dark neutral for light text. Audio controls instead use a shared, translucent five-stop gradient derived from the background palette, with a narrow tinted highlight and subtle edge lighting. SVG fills/strokes paint only the icon shapes; seek, magnifier and speed tracks, markers, thumbs and readouts use matching gradient shading. HSL lightness changes retain the palette's hue and saturation, while translucency lets the underlying gradient show through. There are no glass panels or boxes behind the hit targets. Thin versus thick strokes distinguish tracks and progress. An entirely neutral gradient remains neutral. The settings surface swatch does not change the audio colors.
