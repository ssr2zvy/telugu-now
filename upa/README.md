...
Under **Display > Appearance > Position**, separate text and audio-bar sliders adjust their vertical offsets from the defaults, from -200 to +200 pixels. Negative values move up; positive values move down. The player starts lower, with a bottom inset of 16 pixels plus the device safe area. **Audio control order** explicitly offers **Play above / magnifier below** (default) and **Play below / magnifier above**. Play is centered on the page, and the main seek track spans the player width. A translucent, palette-tinted lens highlights the section of the main bar being magnified and repeats its rail inside the enlarged waveform. Playback speed and bookmark sit in a horizontal row below the lens in either control order, hidden whenever the magnifier is closed. Opening speed keeps the magnifier open and places its slider beside the action row without moving the main seek bar. Space is reserved for precision seeking so opening it does not shift the main bar under the pointer. Routine audio preparation appears as dots in the top-right navigation-feedback position, not a text panel next to the player; genuine errors remain visible. Saved offsets and control order remain intact. Positions are constrained to the viewport, and long text is refitted to avoid overlapping audio controls. **Reset positions** restores both offsets to zero and Play above / magnifier below.
...
Playback speed supports 0.1x-1.5x. Play has an 80 by 64 pixel hit box; speed and bookmarks have 48 by 44 pixel targets in their vertical stack. The precision scrubber moves one millisecond per pointer pixel. The speed popover stays within the player footprint and consumes its outside-dismissal click without navigating. Its neighboring action buttons remain directly usable while speed is open.
The precision bar and its speed/bookmark actions start hidden. A 300 ms hold on the main seek bar, or a pressure-sensitive hard press, opens the group and pauses playback. A normal click still only seeks. Enter/Space on the main bar provides keyboard access; Escape, an outside click, or fading the controls dismisses the entire precision group, including an open speed slider. Pointer interaction or arrow keys pause playback for fine seeking; playback resumes only through Play. The default Play/seek gestures and the 500 ms silent lead-in are unchanged. All icon hit boxes remain large but have no visible squares, fills or borders.
...
Audio is prepared ahead of playback: the current recording has priority, and up to three upcoming ready queue/forward-history recordings are warmed without consuming reservations. Downloads are deduplicated; speculative preparation is serial with a reserved current-clip slot. Each recording is decoded, speech-only loudness-normalized, and encoded into one native PCM WAV Blob with exactly 500 ms of zero samples prepended. The native element plays directly, never through a suspendable Web Audio output graph, and Play does not swap sources. The silent half-second is part of the duration, waveform, and seek bar; animation-frame updates follow the native clock throughout it. The selected pitch-preserving playback rate applies to the entire clip (at 0.5x, the half-second occupies one wall-clock second). Resume continues at the paused position without adding silence; replay starts at zero.

Cold preparation displays a loading status, not a false playing state. If Play is tapped before preparation completes, it asks for another tap when ready, preserving mobile gesture authorization rather than attempting delayed autoplay. Failed downloads, decoding, and playback show actionable feedback; Play retries. Navigation aborts unowned pending downloads, ignores stale completions, and releases Blob ownership only after detaching native media. The client cache retains at most six unowned/owned entries in normal reader use within a 64 MiB WAV budget; in-use Blobs are never evicted. Downloads have a 32 MiB limit and a 30-second preparation deadline; decoded clips support up to two channels and two minutes, with a 16 MiB prepared-clip limit. Decoding resamples to 24 kHz. Web Audio decoding is required, but its context need not run; native decoders cannot be interrupted mid-decode, so abandoned results are discarded. Original corpus files and exports are unchanged. Bookmarks stay in original speech seconds in persistence and are translated by +0.5 seconds only for display/seeking (−0.5 when creating), including retries and reloads.

Focused browser regression command (with the development client running): `npx playwright test tests/ui.browser.spec.ts --grep 'prepared audio|mobile progress follows|fixed precision seeking' --output=test-results/audio-browser`. These mocked tests cover PCM silence, captured native sound, clock tracking, queue warming/cancellation, errors/retries, saved speed, autoplay, rapid navigation, and bookmark round-trips. For device acceptance, open a profile on iOS Safari/Android Chrome, let queued audio warm, then tap Play: the bar must advance immediately through the silent prefix and speech must follow without another source load. Check cold preparation on a throttled connection, pause/resume during silence and speech, backward/forward seeking, 0.1x/0.5x/1.5x speed, replay, navigation, and bookmark reloads. Touch emulation is not a substitute for physical-device audio/pitch verification.
The audio controls and Settings icon start hidden. Reader taps use a 400 ms decision window: a single tap toggles audio only after that window, while a second nearby tap in the same region cancels the single action entirely. Double-clicking or double-tapping blank center space toggles the Settings icon without briefly revealing audio or creating an extra click target between taps. Click the icon to enter Settings. Double taps suppress native text selection, while ordinary cursor/drag selection remains available. Edge double taps still navigate and word double taps still open word profiles. Direct audio-button clicks and keyboard navigation do not wait for the reader-tap window. Keyboard focus can reveal either control group for accessibility. **Display > Appearance > Auto-fade** sets the shared inactivity delay from 1 to 60 seconds (default 15). Movement and keyboard activity reset the timer but do not reveal hidden controls. Active dragging postpones fading until release. Hiding controls does not stop playback. Navigation hides both groups. Profile preference loading uses the same text-free spinner as sign-in instead of flashing a loading sentence.
