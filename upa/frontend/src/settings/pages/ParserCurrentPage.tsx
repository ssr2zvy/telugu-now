...
import { completion, targetUnicode } from '../parser-display';
import { matchedWordEvidence } from './SelectedWordMatch';
import { ParserSection } from './ParserSection';

export function ParserCurrentPage({profileCode,observation}:{profileCode:string;observation:DisplayObservation|null}) {
  ...
  return <div className="parser-settings">
    ...
    {!data ? <p role="status">Loading current progress…</p> : <>
      ...
      <section className="parser-block" aria-label="Loaded chain">
        <div className="parser-line"><h2>Chain</h2>{data.currentChain ? <span>Core {data.currentChain.core} · {data.currentChain.steps.length} steps</span> : null}</div>
        {data.currentChain ? <ol className="parser-chain">{data.currentChain.steps.map(step => {
          const forms = targetUnicode(data.targets.find(target => target.id === step.targetId));
          ...
          return <li key={step.observationId} aria-current={isCurrent ? 'step' : undefined}>
            <span lang={forms.length ? 'te' : undefined}>{forms.join(' · ') || 'Unicode unavailable'}</span>
      ...
      {!data.progressError ? <ParserSection title="Progress by core" summary="Three consecutive correct answers per object">
        ...
        <p className="parser-muted">An incorrect answer resets that object’s streak. Chain entries show Unicode search forms, not a predicted future path.</p>
      </ParserSection> : null}
    </>}
  </div>;
}
