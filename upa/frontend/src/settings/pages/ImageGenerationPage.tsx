...
import { CORE_WORD_PLACEHOLDER, SENTENCE_PLACEHOLDER, IMAGE_MODEL, validImagePrompt, type ImageSettings } from '../../../../shared/image-settings';
...
export function ImageGenerationPage({ language }: { language: UiLanguage }) {
  ...
  return (
    <form className="image-generation-settings" onSubmit={event => { event.preventDefault(); void save(); }}>
      ...
      <p id="image-prompt-placeholder" className="image-prompt-requirement">{english ? 'Required placeholder' : 'అవసరమైన గుర్తు'}: <code>{CORE_WORD_PLACEHOLDER}</code>. {english ? 'Optional current sentence' : 'ఐచ్ఛిక ప్రస్తుత వాక్యం'}: <code>{SENTENCE_PLACEHOLDER}</code></p>
