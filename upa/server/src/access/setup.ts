...
generate.onclick=async()=>{if(generating)return;generating=true;generate.disabled=true;confirm.disabled=true;status.textContent='';try{const response=await fetch('/access/generate',{method:'POST',cache:'no-store'});if(response.status===409){location.replace('/access');return;}if(!response.ok)throw new Error();const result=await response.json();phrase.value=result.phrase;phrase.type='text';reveal.setAttribute('aria-pressed','true');reveal.setAttribute('aria-label','దాచు');}catch{status.textContent='మళ్లీ ప్రయత్నించండి.';}finally{generating=false;generate.disabled=false;confirm.disabled=false;}};
