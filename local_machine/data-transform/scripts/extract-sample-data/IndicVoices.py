...
DEFAULT_INPUT_ROOT = Path(__file__).resolve().parents[4] / "data/raw/IndicVoices"
DEFAULT_OUTPUT_ROOT = Path(__file__).resolve().parents[4] / "data/sample/IndicVoices"
DEFAULT_PARQUETS = ["train-00000-of-00061.parquet"]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract sampled IndicVoices Telugu Parquet rows."
    )
    parser.add_argument(
        "--input-root",
        type=Path,
    ...
    if args.all_parquets:
        ...
        if not parquet_paths:
            if args.all_rows and any(args.output_root.rglob("*.parquet")):
                print("IndicVoices: no remaining raw shards; keeping existing samples")
                return
